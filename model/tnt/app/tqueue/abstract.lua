...
local session  = require('app.tqueue.abstract.queue_session')
local state    = require('app.tqueue.abstract.state')
local queue_state = require('app.tqueue.abstract.queue_state')

local util     = require('app.tqueue.util')
local qc       = require('app.tqueue.compat')
...
-- MODIFIED RABBIT METHODS
-- find_task(utube, order_id)
function tube.find_task(self, profile_id, order_id)
    if not check_state() then
        return nil
    end
    local task = self.raw:find_task(profile_id, order_id)
    if task == nil then
...
-- delete_by_utube(utube)
function tube.delete_by_profile_id(self, profile_id)
    if not check_state() then
        return nil
    end
    self.raw:delete_by_profile_id(profile_id)
    return nil
end
